...
// 모든 요청방식을 처리하는 유틸함수
const fetchPromiseData = (method, url, payload) => {
...
fetchPromiseData("GET", URL).then((xhr) => {
...
$addForm.addEventListener("submit", (e) => {
  ...
  fetchPromiseData('POST', URL, payload).then(xhr => {
...
const deletePostHandler = (e) => {
  ...
  fetchPromiseData('DELETE', `${URL}/${id}`).then(xhr => {
    if (xhr.status === 200) {
      alert("삭제 성공!");
    } else {
      alert("삭제 실패!");
